...
slist* create(){
    ...
    scanf("%d",&tot);
    printf("Input 1 surname: ");
    scanf("%s",prev->surname);
    printf("Input grades for %s: ",prev->surname);
    for(int j=0;j<N;j++) scanf("%d",&prev->grades[j]);
    prev->dele=0;
    while(i<tot){
        cur=new slist;
        printf("Input %d surname: ",i+1);
        scanf("%s",cur->surname);
        printf("Input grades for %s: ",cur->surname);
...
void pl(slist *head){
    ...
    while (cur!=NULL){
        if(!cur->dele){
            printf("%s's grades are ",cur->surname);
